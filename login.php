<?php
session_start();
session_unset();
$error=0;
require ("connect.php");
$conn = CreateHandle();
if (isset($_POST['cmdSubmit'])){

$email = isset($_POST['email']) ? $_POST['email'] : "";
$pass = isset($_POST['upass']) ? $_POST['upass'] : "";
    
$email = $conn->real_escape_string($email);
$pass = $conn->real_escape_string($pass);    
    
$sqlquery = "SELECT * FROM users WHERE email = '$email'";
    
    if (isset($conn)){
    $result = $conn->query($sqlquery);
    $userdetails = $result->fetch_array(MYSQLI_NUM);
        
        if(isset($userdetails)){
            
            if($userdetails[3] == $pass){
                $_SESSION['name'] = $userdetails[2];
                $_SESSION['email'] = $userdetails[1];
                $_SESSION['userID'] = $userdetails[0];
                header('location: index.php?loggedin=1');
                
            }else{
               $error=2; //password wrong
            }
            
        }else{
            $error=1; //email wrong
        }
    }

}


?>

<!doctype html>
<html lang="en">

<head>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">

    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="style.css">
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css" integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T" crossorigin="anonymous">
    <link href="https://fonts.googleapis.com/css?family=Orbitron" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css?family=Abel" rel="stylesheet">

    <title>PK Weather Login!</title>
</head>
    
<body>

    <div class="container">
        <div class="wrapper">
            <form class="form-signin" id="loginDetails" name="loginDetails" action="<?php echo $_SERVER['PHP_SELF']?>" method="post">
                <br>
                <h1 class="text-center">Login!</h1>
                <br>
                <input class="form-control" type="email" id="email" name="email" required="" autofocus="" placeholder="Email" value="" />
                <br>
                <input class="form-control" type="password" id="upass" name="upass" required="" placeholder="Password" />
                
                <?php
                    if($error == 1){
                        echo "<p>Email is wrong or account does not exist</p>";
                    }
                  if($error == 2){
                        echo "<p>Wrong Password</p>";
                    }
                  if(isset($_GET['redirect']) &&  $_GET['redirect'] == 1){
                        echo "<p>You need to be logged in</p>";
                    }
                ?>
                <br>
                <button class="btn btn-lg btn-primary btn-block" type="submit" id="cmdSubmit" name="cmdSubmit" value="sign in">Login</button>
                <br>
                <a class="btn btn-lg btn-danger btn-block" href="register.php">Register?</a>
                
                
                
            </form>
             
            
        </div>


    </div> <!-- /container -->





    <!-- Optional JavaScript -->
    <!-- jQuery first, then Popper.js, then Bootstrap JS -->
    <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js" integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js" integrity="sha384-UO2eT0CpHqdSJQ6hJty5KVphtPhzWj9WO1clHTMGa3JDZwrnQq4sF86dIHNDz0W1" crossorigin="anonymous"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js" integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM" crossorigin="anonymous"></script>
</body>

</html>
